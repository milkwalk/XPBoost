...
public class Main extends JavaPlugin {
	...
	public void onEnable() {
		...
		if (!Main.getPlugin().getConfig().contains("settings.config_version")
				|| Main.getPlugin().getConfig().getInt("settings.config_version") < Main.config_version) {
			this.log.warning(
					"&cYou config is out of date, regenerate you config fille or add required keys for the full functionality of the plugin. You version: &a"
							+ Main.getPlugin().getConfig().getInt("settings.config_version") + " &cnewest version: &a"
							+ Main.config_version);
		}

		// language
		if (getConfig().contains("settings.language")) {
			langFile = new File(Main.getPlugin().getDataFolder() + "/lang/lang_"
